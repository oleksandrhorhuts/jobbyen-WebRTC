...
class ISSJob extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cronjob:ISSJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'ISS';
        $base_url = 'https://www.dk.issworld.com';
        ...
        $crawler = $goutte->request('GET', 'https://candidate.hr-manager.net/vacancies/list.aspx?customer=iss');
        ...
        foreach (json_decode($values[0])->PositionList->Items as $index => $node) {
            if ($current_pagination - 1 < $index && $current_pagination + 2 >= $index) {
                ...
                $contact_person = '';
                if ($node->CustomText1) {
                    $contact_person = $node->CustomText1;
                }
                $this->info('Job Contact Person :' . $contact_person);
                $contact_phone = '';
                if ($node->CustomText2) {
                    $contact_phone = $node->CustomText2;
                }
                ...
                // $this->info('Description : '.$job_description);
                $this->info('================================================');
