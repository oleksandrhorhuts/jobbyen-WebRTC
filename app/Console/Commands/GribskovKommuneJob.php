...
class GribskovKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:GribskovKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Gribskov Kommune';
        $base_url = 'https://www.gribskov.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://signatur.frontlab.com/ExtJobs/DefaultHosting/JobList.aspx?ClientId=2079');
        $total_pagination = $crawler->filter('.job-list-classic tr')->count();
        ...
        $data_25 = $crawler->filter('.job-list-classic tr')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index && $index > 0) {
                ...
                $job_deadline_date = $node->filter('td:nth-child(1) .deadline-xs')->text();
                ...
                $job_description = $crawler->filter('#ctl00_mainContent_contentBodyOuterDiv')->text();
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Gribskov Kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Gribskov Kommune';
