...
class AalborgKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:AalborgKommuneJob';
    ...
    public function handle()
    {
        ...
        $crawler = $goutte->request('GET', 'https://www.aalborg.dk/job-og-ledighed/job');
        $logo = '';
        $logoSource = $crawler->filter('#logo img')->extract(array('src'));
        print_r($logoSource);
        $logoSource = isset($logoSource[0]) ? $base_url . $logoSource[0] : '';
        ...
        $data_25 = $crawler->filter('.jobs--singlejob')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr) {

            if ($current_pagination - 1 < $index && $current_pagination + 9 >= $index) {
                $job_url = $node->filter('.singlejob--description')->extract(['src']);
                ...
                if($crawler->filter('#main-text')->count()){
                    $job_description = trim($crawler->filter('#main-text')->text());
                    $job_description = preg_replace("/\r|\n/", "", $job_description);
                    $job_description = preg_replace("/\t/", " ", $job_description);
                    $job_description = trim($job_description, " \t\n\r\0\x0B\xC2\xA0");

                    $job_description_html_string = $crawler->filter('#main-text')->html();
        ...
        foreach ($data as $value) {
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                $new_job->company = 'Aalborg kommune';
