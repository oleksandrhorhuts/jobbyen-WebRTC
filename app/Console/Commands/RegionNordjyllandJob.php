...
class RegionNordjyllandJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:RegionNordjyllandJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Region Nordjylland';
        $base_url = 'https://rn.dk';
        ...
        $crawler = $goutte->request('GET', 'https://rn.dk/genveje/karriere/ledige-job');
        $logo = '';

        $total_pagination = $crawler->filter('.list-container ul li .list-item')->count();
        ...
        $data_25 = $crawler->filter('.list-container ul li .list-item')->each(function ($node, $index) use ($base_url, $logo, $current_pagination, $find_arr, $replace_arr) {

            if ($current_pagination - 1 <= $index && $current_pagination + 4 > $index) {
                $job_url = $node->filter('.list-header .list-title a')->extract(['href']);
                $job_url = isset($job_url[0]) ? $job_url[0] : '';

                $job_title = trim($node->filter('.list-header .list-title a')->text());
                ...
                if ($crawler->filter('.module-job-description .module-content .module-text div:nth-child(2) p')->count() > 0) {
                    $work_address = $crawler->filter('.module-job-description .module-content .module-text div:nth-child(2) p')->text();
                } else {
                    $work_address = "";
                }

                $work_city = "";
                if ($crawler->filter('.module-job-description .module-content .module-text div:nth-child(1) span')->text() == 'Arbejdssted') {
                    if ($crawler->filter('.module-job-description .module-content .module-text div:nth-child(1) p')->count()) {
                        $work_city = $crawler->filter('.module-job-description .module-content .module-text div:nth-child(1) p')->text();
                    } else {
                        $work_city = "";
                    }
                }

                if ($work_city == 'Aalborg Universitetshospital') {
                    $work_city = 'Aalborg';
                } else if ($work_city == 'Regionshuset') {
                    $work_city = '';
                } else if ($work_city == 'Region Nordjylland') {
                    $work_city = 'Nordjylland';
                } else if ($work_city == 'Psykiatrien') {
                    $work_city = '';
                } else if ($work_city == 'Specialsektoren') {
                    $work_city = '';
                } else if ($work_city == 'Selvejende institution') {
                    $work_city = '';
                } else {
                    $work_city = '';
                }
                ...
                $job_deadline_date = trim($node->filter('.list-content table tr:nth-child(3) td:nth-child(2)')->text());
                $job_deadline_date = str_replace($find_arr, $replace_arr, $job_deadline_date);

                $ddate = date("Y-m-d", strtotime($job_deadline_date));
                if ($ddate == "1970-01-01") {
                    $job_deadline_date = "";
                } else {
                ...
                $work_type = $node->filter('.list-content table tr:nth-child(2) td:nth-child(2)')->text();
                ...
                if ($crawler->filter('.print-span9 div .row .page-content-content')->count() > 0) {
                    $job_description = trim($crawler->filter('.print-span9 div .row .page-content-content')->text());

                    $job_description = preg_replace("/\r|\n/", "", $job_description);
                    $job_description = preg_replace("/\t/", " ", $job_description);
                    $job_description = trim($job_description, " \t\n\r\0\x0B\xC2\xA0");

                    $job_description_html_string = $crawler->filter('.print-span9 div .row .page-content-content')->html();
        ...
        foreach ($data as $value) {
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                $job_logo = 'region-nordjylland.png';
                ...
                $new_job->company = 'Region Nordjylland';
