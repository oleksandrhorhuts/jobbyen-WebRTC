...
class KoldingKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:KoldingKommuneJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Kolding kommune';
	   	$base_url = 'https://www.kolding.dk';
    	...
    	$crawler = $goutte->request('GET', 'https://www.kolding.dk/ledige-stillinger/stillingsopslag');
    	$logo = '';

        $goutte = new Client();
        // $goutte->setClient(new GuzzleClient(array('verify' => false)));
		$crawler = $goutte->request('GET', 'https://candidate.hr-manager.net/vacancies/list.aspx?customer=ofir_kolding');

		$total_pagination = $crawler->filter('.ProjectList tr')->count();
	    ...
	    $data_25 = $crawler->filter('.ProjectList tr')->each(function ($node, $index) use ($base_url, $logo, $find_arr, $current_pagination, $replace_arr){

	        if($current_pagination - 1 <= $index && $current_pagination + 4 > $index)
	        {
                $nodeClass = $node->extract(['class']);
                $nodeClass = isset($nodeClass[0]) ? $nodeClass[0] : '';
	            if($nodeClass=="ProjectList_Row" || $nodeClass=="ProjectList_AlternatingRow")
    			{

    				$job_url = "";
                    $job_url_source = $node->filter('td:nth-child(2) .ProjectList_ProjectLink')->extract(['href']);
                    if(isset($job_url_source[0])){
    				    $job_url = "https://candidate.hr-manager.net/". $job_url_source[0];
                    }

    				$job_title = trim($node->filter('td:nth-child(2) .ProjectList_ProjectLink')->text());
    		    	...
    		    	$ddate=date("Y-m-d",strtotime(trim($node->filter('td:nth-child(3)')->text())));
    		    	...
    		    	$job_description = trim($crawler->filter('#AdvertisementInnerContent')->text());
	    ...
	    $data = array_values(array_filter(array_merge($data, $data_25)));
    	    ...
    	    if(isset($value['Job Title']) && $value['Job Title'] !='')
    	    {
    			...
    			if($work_city != '')
    			{
                    ...
                    else
                    {
                        ...
                        $place_model->company = 'Kolding kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Kolding kommune';
