...
class FredensborgKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:FredensborgKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'FREDENSBORG KOMMUNE';
        $base_url = 'https://www.fredensborg.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://www.fredensborg.dk/kommunen/job-i-kommunen/ledige-job-i-kommunen');
        $total_pagination = $crawler->filter('.accordeon-list .panel-default table tr')->count();
        ...
        $data_25 = $crawler->filter('.accordeon-list .panel-default table tr')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                ...
                if($node->filter('td:nth-child(1) a')->count()){
                    ...
                    $job_url = 'https://www.fredensborg.dk'.$job_url;

                    $job_title = $node->filter('td:nth-child(1) a')->text();

                    $job_deadline_date = '';

                    $deadline = $node->filter('td:nth-child(2)')->text();

                    $dateArr = explode("kl.", $deadline);
                    if(sizeof($dateArr) > 1){
                        $temp = trim($dateArr[0], " \t\n\r\0\x0B\xC2\xA0");
                        $job_deadline_date = str_replace($find_arr, $replace_arr, $temp);
                    }
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'FREDENSBORG KOMMUNE';
                        if ($place_model->save()) {
                ...
                $job_logo = 'fredensborg_kommune.png';
                ...
                $new_job->company = 'FREDENSBORG KOMMUNE';
