...
class HviidLarsenApsJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HviidLarsenApsJob';
    ...
    public function handle()
    {
        ...
        $crawler = $goutte->request('GET', 'https://recruit.hr-on.com/frame-api/pages/list-jobs?theme=HviidLarsen-v2&joblisturl=https%3A%2F%2Fwww.hviidoglarsen.dk%2Fjobs&includeArchived=true&locale=da_DK&companyid=204&xdmframe=1&metaTitle=Jobs%20%7C%20Hviid%20%26%20Larsen');
        $total_pagination = $crawler->filter('.jobposting:not(.archived)')->count();
        ...
        $data_25 = $crawler->filter('.jobposting:not(.archived)')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {

            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {

                $job_title = $node->filter('.left .content-header')->text();
                $job_url = $node->filter('.left .content-header a')->extract(['href']);
                $job_url = isset($job_url[0]) ? $job_url[0] : '';
                $job_description = $node->filter('.left .jobdesc')->text();

                $goutte = new Client();
                // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                $crawler = $goutte->request('GET', $job_url);

                $job_description_html = $crawler->filter('.description')->html();

                $contact_email = '';
                if($crawler->filter('.info_column .contacts .block')->count()){
                    if($crawler->filter('.info_column .contacts .block a')->count()){
                        $contact_email = $crawler->filter('.info_column .contacts .block a')->text();
                    }
                }

                // $work_city = $node->filter('.left .job-info .worklocation span:nth-child(2)')->text();
                // $job_created_date = $node->filter('.left .job-info .worklocation.pdate span:nth-child(2)')->text();

                // $job_created_date = str_replace($find_arr, $replace_arr, $job_created_date);
                // $job_deadline_date = 'snarest muligt';

                // $job_info = $node->filter('.left .job-info')->text();
			...
			if (isset($value['Job Title']) && $value['Job Title'] != '') {
				...
				if ($work_city != '') {
					...
					if ($city) {
						...
						$place_model->company = 'Hviid Larsen ApS';
						if ($place_model->save()) {
				...
				$new_job->company = 'Hviid Larsen ApS';
