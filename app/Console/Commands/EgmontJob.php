...
class EgmontJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:EgmontJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Egmont Group';
	   	$base_url = 'https://www.ioi.dk';
    	...
    	$crawler = $goutte->request('GET', 'https://www.ioi.dk/jobs/');
		$logo = '';

	    $total_pagination = $crawler->filter('.vc_col-sm-4')->count();
	    ...
	    $data_25 = $crawler->filter('.vc_col-sm-4')->each(function ($node, $index) use ($base_url, $logo, $find_arr, $current_pagination,  $replace_arr){
            ...
            if($current_pagination - 1 <= $index && $current_pagination + 1 > $index)
	        {
				...
				$job_title = $node->filter('.entry-header')->text();
            	...
            	$job_description = $crawler->filter(".container-wrap")->text();

                $job_description = preg_replace("/\r|\n/", "", $job_description);
	            $job_description = preg_replace("/\t/", " ", $job_description);
	            $job_description = trim($job_description, " \t\n\r\0\x0B\xC2\xA0");

            	$job_description_html_string = $crawler->filter(".container-wrap")->html();
                ...
                $job_description_html_string = preg_replace('/<div class="container main-content">(.*)<\/div>/', "$1", $job_description_html_string);
