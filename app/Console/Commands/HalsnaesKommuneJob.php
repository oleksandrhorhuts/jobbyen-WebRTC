...
class HalsnaesKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HalsnaesKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Halsnæs Kommune';
        $base_url = 'https://halsnaes.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://halsnaes.emply.net/overview/halsnaes.aspx?mediaid=537a7324-00fa-42c1-822d-d0256092ddb9');
        $total_pagination = $crawler->filter('.jobs_table tr')->count();
        ...
        $data_25 = $crawler->filter('.jobs_table tr')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Halsnæs Kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Halsnæs Kommune';
