...
class NovoNordiskJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:NovoNordiskJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Novo Nordisk';
	   	$base_url = 'https://www.novonordisk.dk';
		...
		$crawler = $goutte->request('GET', 'https://www.novonordisk.dk/content/Denmark/AFFILIATE/www-novonordisk-dk/da_dk/hjem/karriere/ledige-jobs.html?keyword=&country=Denmark&category=All');
		$logo = $crawler->filter('.header-logo')->extract(['src']);
        $logo = isset($logo[0]) ? $base_url . $logo[0] : '';
		$client = new Client();
        // $client->setClient(new GuzzleClient(array('verify' => false)));
		$crawler = $client->request('GET', 'https://www.novonordisk.dk/bin/nextweb/jobsearch/get-results?keyword=&country=Denmark&category=All&descount=100');
		$jobs_data=json_decode($client->getResponse()->getContent());


		$total_pagination = sizeof($jobs_data->jobs);
	    ...
	    foreach ($jobs_data->jobs as $index=>$job_data)
	    {
		    if($current_pagination - 1 <=$index && $current_pagination + 2 > $index)
            {
                $job_created_date = $job_data->postedDate;
                $goutte = new Client();
                // $goutte->setClient(new GuzzleClient(array('verify' => false)));
    	    	$crawler = $goutte->request('GET', $job_data->jobDetailLink);
        		$job_description_html = $crawler->filter('span.jobdescription p')->each(function($node, $index){
    	            if(trim($node->text(), " \t\n\r\0\x0B\xC2\xA0") !='')
    	            {
                        return $node->html();
    	            }
    	    	...
    	    	$temp_data = array(
    				...
    				"Job Created Date" => date('Y-m-d', strtotime($job_data->postedDate)),
    				"Job Start Date" => '',
    				"Job Deadline Date" => '',
    				"Company Logo" => $logo,
    				"Work address" => $job_data->jobLocation,
    				"Work City" => $job_data->jobLocation,
    				"Work Type" => '',
    				"Job Type" => 1,
    				"Job Url" => $job_data->jobDetailLink,
    	    ...
    	    if(isset($value['Job Title']) && $value['Job Title'] !='')
    	    {
    			...
    			if($work_city != '')
    			{
                    ...
                    else
                    {
                        ...
                        $place_model->company = 'Novo Nordisk';
                        if ($place_model->save()) {
