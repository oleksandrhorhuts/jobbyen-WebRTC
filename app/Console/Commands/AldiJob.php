...
class AldiJob extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cronjob:AldiJob';
    ...
    public function handle()
    {
    	...
    	$crawler = $goutte->request('GET', 'https://www.aldi.dk/karriere/ledige_stillinger.html');
		$logo='';
        $logoSource = $crawler->filter('.mod-header__logo img')->extract(['src']);
		if(isset($logoSource[0])) $logo=$base_url. $logoSource[0];

        $goutte = new Client();
        // $goutte->setClient(new GuzzleClient(array('verify' => false)));
		$crawler = $goutte->request('GET', 'https://www.aldi.dk/mds/hr/api/jobSearch/21');
		$jobs_data=json_decode($goutte->getResponse()->getContent());
	    ...
	    foreach ($jobs_data->jobs as $index=>$job_data)
	    {
	        if($current_pagination - 1 < $index && $current_pagination + 5 >=$index)
            {
                ...
                $job_url="https://www.aldi.dk/karriere/jobopslag.".$job_data->postingId.".html/".$job_data->slugTitle.".html";
                $goutte = new Client();
                // $goutte->setClient(new GuzzleClient(array('verify' => false)));
    			$crawler = $goutte->request('GET', $job_url);
    			$job_description=trim(strip_tags($crawler->filter(".mod-job-detail__body")->text()));

                $job_description = preg_replace("/\r|\n/", "", $job_description);
                $job_description = preg_replace("/\t/", " ", $job_description);
                $job_description = trim($job_description, " \t\n\r\0\x0B\xC2\xA0");

    			$job_description_html=$crawler->filter(".mod-job-detail__body")->html();
    			...
    			$job_created_date = date("Y-m-d",strtotime($job_data->onlineFrom));
    	    	$job_deadline_date = date("Y-m-d",strtotime($job_data->onlineUntil));
    	    	...
    	    	$temp_data = array(
    				...
    				"Company Logo" => $logo,
    				"Work address" => $locations->country,
    				"Work City" => $locations->city,
    				"Work Type" => '',
    				"Job Type" => 1,
    				"Job Url" => $job_url,
