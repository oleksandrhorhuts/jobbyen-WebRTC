...
class IkastBrandeKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:IkastBrandeKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Ikast-Brande Kommune';
        $base_url = 'https://ikast-brande.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://ikast-brande.dk/job/ledige-stillinger-i-ikast-brande-kommune/ledige-stillinger-i-ikast-brande-kommune');
        ...
        $data_25 = $crawler->filter('.emplylist .card__list')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                ...
                $contact_email = $this->cutString($sub_details, '<b>E-mailadresse:', '<br><b>');
                if($contact_email){
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Ikast-Brande Kommune';
                        if ($place_model->save()) {
                ...
                $job_logo = 'ikast_brande_kommune.png';
                ...
                $new_job->company = 'Ikast-Brande Kommune';
