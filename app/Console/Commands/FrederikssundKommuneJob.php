...
class FrederikssundKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:FrederikssundKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'FREDERIKSSUND KOMMUNE';
        $base_url = 'https://www.frederikssund.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://www.frederikssund.dk/service/Job/Ledige-stillinger');
        $total_pagination = $crawler->filter('#cs-content .table-styled-3 tbody tr')->count();
        ...
        $data_25 = $crawler->filter('#cs-content .table-styled-3 tbody tr')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                if($node->filter('td:nth-child(1) a')->count()){
                    $job_url = $node->filter('td:nth-child(1) a')->extract(['href']);
                    $job_url = isset($job_url[0]) ? $job_url[0] : '';

                    $job_title = $node->filter('td:nth-child(1) a')->text();

                    $job_deadline_date = $node->filter('td:nth-child(2)')->text();

                    $job_deadline_date = str_replace($find_arr, $replace_arr, $job_deadline_date);

                    $job_url = 'https://www.frederikssund.dk/service/Job/Ledige-stillinger'.$job_url;

                    $goutte = new Client();
                    // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                    $crawler = $goutte->request('GET', $job_url);

                    $job_description_html = $crawler->filter('#cs-content')->html();
                    $job_description_html = preg_replace('#<div class=\"pull-right\">.*?</div>#is', '', $job_description_html);
                    $job_description_html = preg_replace('/<a href=\"\?\">(.*)<\/a>/', '', $job_description_html);
                    $job_description_html = preg_replace('/<h1>(.*)<\/h1>/', '', $job_description_html);

                    $job_description = strip_tags($job_description_html);

                    $work_city = 'Frederikssund';
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'FREDERIKSSUND KOMMUNE';
                        if ($place_model->save()) {
                ...
                $job_logo = 'frederikssund_kommune.png';
                ...
                $new_job->company = 'FREDERIKSSUND KOMMUNE';
