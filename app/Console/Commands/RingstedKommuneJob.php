...
class RingstedKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:RingstedKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Ringsted Kommune';
        $base_url = 'https://ringsted.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://ringsted.emply.net/overview/Ringsted.aspx?mediaId=f1fb8e12-1928-4328-88fd-9918a34a2bf2');
        $total_pagination = $crawler->filter('table.jobs_table tr')->count();
        ...
        $data_25 = $crawler->filter('table.jobs_table tr')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Ringsted Kommune';
                        if ($place_model->save()) {
                            $place_inserted_id = $place_model->id;
                        }
                    }
                }
                $job_logo = 'ringsted_kommune.png';
                ...
                $new_job->company = 'Ringsted Kommune';
