...
class DanskeBankJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:DanskeBankJob';
    ...
    public function handle()
    {
		...
		$crawler = $goutte->request('GET', 'https://danskebank.com/da-dk/karriere/soeg-job/Pages/VacantJobs.aspx');
		$logo = " ";

		$total_pagination = $crawler->filter('.gsOverflow table#ctl00_m_g_d6b11528_9bb5_4f9f_9f63_269ab798480b_TopmostDynamicTable tr:nth-child(4) td div table tr')->count();
	    ...
	    $data_25 = $crawler->filter('.gsOverflow table#ctl00_m_g_d6b11528_9bb5_4f9f_9f63_269ab798480b_TopmostDynamicTable tr:nth-child(4) td div table tr')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr) {
			$no_rec = $node->filter('td')->text();
			if($no_rec!="Jobtitel")
			{
				if($node->filter('td a')->count() > 0)
				{
        	        ...
        	        {
        	            $job_url = $node->filter('td a')->extract(['onclick']);
                        $job_url = isset($job_url[0]) ? $job_url[0] : '';
    					$job_url = str_replace("javascript:window.open('","",$job_url);
    					$job_url = str_replace("','_self'); return false;","",$job_url);
    					...
    					$job_title = explode("     ", $job_title)[0];

    			    	$job_created_date = '';
    			    	$job_type = $node->filter('td:nth-child(3) span')->text();

    			    	$address = $node->filter('td:nth-child(2) span')->text();;
    			    	$work_address = $node->filter('td:nth-child(2) span')->text();
    			    	$work_city = $node->filter('td:nth-child(2) span')->text();


    			    	$job_start_date = '';
    			    	$job_deadline_date = date("Y-m-d",strtotime($node->filter('td:nth-child(5) span')->text()));
    			    	$work_type = '1';
    			    	...
    			    	$job_description = $crawler->filter('div.gsDbgDynamicTablePaddingLeft table tr:nth-child(5) td table:nth-child(2) tr td div table tr td div table tr')->text();
    			    	$job_description = preg_replace("/\r|\n/", "", $job_description);

    			    	$job_description_html = $crawler->filter('div.gsDbgDynamicTablePaddingLeft table tr:nth-child(5) td table:nth-child(2) tr td div table tr td div table tr')->html();
	    ...
	    foreach($data as $value){
    	    if(isset($value['Job Title']) && $value['Job Title'] !='')
    	    {
    			...
    			if($work_city != '')
    			{
                    ...
                    } else if($work_city == 'Copenhagen'){
                        $work_city = 'København';
                    } else if($work_city == 'Høje Taastrup'){
                        $work_city = 'Taastrup';
                    } else if($work_city == 'Ejby, Glostrup'){
                        $work_city = 'Glostrup';
                    } else if($work_city == 'Lersø Park Allé'){
                        $work_city = 'København';
                    }
                    ...
                    else
                    {
                        ...
                        $place_model->company = 'Danske Bank';
                        if ($place_model->save()) {
