...
class SlagelseKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:SlagelseKommuneJob';
    ...
    public function handle()
    {
        ...
        $crawler = $goutte->request('GET', 'https://www.slagelse.dk/kommunen/som-arbejdsplads/soeg-job');
        $total_pagination = $crawler->filter('.accordion-item:not(.subscribe)')->count();
        ...
        $data_25 = $crawler->filter('.accordion-item:not(.subscribe)')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {

                $job_deadline_date = $node->filter('.event-date')->text();
                $job_title = $node->filter('.event-title')->text();

                if($node->filter('.accordion-hidden-content a.read-more-link')->count()){
                    $temp_url = $node->filter('.accordion-hidden-content a.read-more-link')->extract(['href']);
                    $temp_url = isset($temp_url[0])? $temp_url[0] : '';

                    $goutte = new Client();
                    // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                    $crawler = $goutte->request('GET', $temp_url);

                    $domain = parse_url($temp_url, PHP_URL_HOST);

                    $job_url = $crawler->filter('#ctl00_mainContent_topActionJobDetailsDiv a')->extract(['href']);
                    $job_url = isset($job_url[0]) ? $job_url[0] :'';
                    $job_url = 'https://' . $domain . $job_url;

                    $goutte = new Client();
                    // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                    $crawler = $goutte->request('GET', $job_url);

                    $job_description_html = $crawler->filter('#ctl00_mainContent_contentBodyOuterDiv')->count();
                    ...
                    $category_crawler = $client->request('GET', $job_category_url);
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'SLAGELSE KOMMUNE';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'SLAGELSE KOMMUNE';
