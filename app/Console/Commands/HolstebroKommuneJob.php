...
class HolstebroKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HolstebroKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'HOLSTEBRO KOMMUNE';
        $base_url = 'https://www.holstebro.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://www.holstebro.dk/job');
        $total_pagination = $crawler->filter('.hk-custom-job-wrapper')->count();
        ...
        $data_25 = $crawler->filter('.hk-custom-job-wrapper')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                $job_url = $node->extract(['href']);
                $job_url = isset($job_url[0]) ? $job_url[0] : '';
                $job_url = 'https://www.holstebro.dk'.$job_url;
                $job_title = $node->filter('h3')->text();
                $job_deadline_date = $node->filter('.hk-custom-job-small')->text();

                $deadlineArr = explode("Deadline:", $job_deadline_date);
                if(sizeof($deadlineArr) > 1){
                    $job_deadline_date = trim($deadlineArr[1], " \t\n\r\0\x0B\xC2\xA0");
                }
                ...
                $job_description_html = $crawler->filter('.hk-custom-leftmenu-template .row div:nth-child(3)')->html();
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'HOLSTEBRO KOMMUNE';
                        if ($place_model->save()) {
                ...
                $job_logo = 'holstebro_kommune.png';
                ...
                $new_job->company = 'HOLSTEBRO KOMMUNE';
