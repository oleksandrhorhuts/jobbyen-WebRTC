...
class CowiJob extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cronjob:CowiJob';
    ...
    public function handle()
    {
        ...
        $crawler = $goutte->request('GET', 'https://www.cowi.com/careers/open-vacancies?jobcountry=Denmark&joblocation=any&jobcategory=any');


        $full_text = mb_convert_encoding($crawler->filter('.parallax script')->text(), 'UTF-8', 'HTML-ENTITIES');
        $jobs_data = json_decode(str_replace("var all_job_items =", "", $full_text));
        ...
        foreach ($jobs_data as $job_data) {
            if ($job_data->Country == 'Denmark') {
                $total_pagination++;
                $denmark_jobs[] = $job_data;
            }
        }
        ...
        //https://www.cowi.com/careers/open-vacancies/job-detail?reqid=31380
        foreach ($denmark_jobs as $index => $job_data) {
            $goutte = new Client();
            // $goutte->setClient(new GuzzleClient(array('verify' => false)));
            $crawler = $goutte->request('GET', 'https://www.cowi.com/careers/open-vacancies/job-detail?reqid=' . $job_data->ReqId);

            $job_url = 'https://www.cowi.com/careers/open-vacancies/job-detail?reqid=' . $job_data->ReqId;

            $temp = $crawler->filter('div.module--padding-bottom-200 .small-12 tbody')->each(function ($node, $index) {
            ...
            $temp = $crawler->filter('div.module--padding-bottom-200 .small-12 tbody')->each(function ($node) {
                return $node->html();
            });


            if (sizeof($temp) > 0) {
                $job_description_html_string = $temp[0];
            }


            $job_parent_category = '';
            $job_sub_category = '';
            $job_category_url = "https://www.jobsearch.dk/jobdatabase?searchText=" . $job_data->Name;
            $goutte = new Client();
            // $goutte->setClient(new GuzzleClient(array('verify' => false)));
            ...
            $temp_data = array(
                ...
                "Job Created Date" => date('Y-m-d', strtotime($job_data->PublishDate)),
                "Job Start Date" => '',
                "Job Deadline Date" => date('Y-m-d', strtotime($job_data->ClosingDate)),
                "Company Logo" => '',
                "Work address" => '',
                "Work City" => $job_data->Locations[0],
                "Work Type" => '1',
                "Job Type" => '',
                "Job Url" => 'https://www.cowi.com/careers/open-vacancies/job-detail?reqid=' . $job_data->ReqId,
        ...
        foreach ($data as $value) {
            ...
            if ($job_title != '') {
                ...
                if (!$existing_job) {
                    ...
                    // $old = true;
                    $updated = Job::where('real_url', $job_url)->update(['title'=>$job_title]);
                }
            }
        }
