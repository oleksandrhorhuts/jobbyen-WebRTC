...
class SkiveKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:SkiveKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'SKIVEKOMMUNE';
        $base_url = 'https://www.skive.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://www.skive.dk/servicemenu/job/');
        $total_pagination = $crawler->filter('.content__rte div:not([class])')->count();
        ...
        $data_25 = $crawler->filter('.content__rte div:not([class])')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                if($node->filter('a')->count()){
                    ...
                    $dd_date = $node->filter('.text-right')->text();
                    $dd_date_arr = explode("Ansøgningsfrist:", $dd_date);

                    $job_deadline_date = '';
                    if(sizeof($dd_date_arr) > 1){
                        $job_deadline_date = $dd_date_arr[1];
                        $job_deadline_date = trim($job_deadline_date, " \t\n\r\0\x0B\xC2\xA0");
                        $job_deadline_date = str_replace($find_arr, $replace_arr, $job_deadline_date);
                    }

                    $job_url = 'https://www.skive.dk'.$job_url;
                    $goutte = new Client();
                    // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                    $crawler = $goutte->request('GET', $job_url);

                    $job_description_html = $crawler->filter('.content__rte .col-sm-12:nth-child(2) div:nth-child(1)')->html();
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'SKIVEKOMMUNE';
                        if ($place_model->save()) {
