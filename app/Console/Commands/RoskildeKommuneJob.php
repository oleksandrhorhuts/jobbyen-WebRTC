...
class RoskildeKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:RoskildeKommuneJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Roskilde kommune';
	   	$base_url = 'https://roskilde.dk';
		...
		$crawler = $goutte->request('GET', 'https://candidate.hr-manager.net/vacancies/list.aspx?customer=roskildekommune');
	    ...
	    $data_25 = $crawler->filter('.ProjectList tr')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr){
			if($current_pagination - 1 <= $index && $current_pagination + 4 > $index)
	        {
    			...
    			{
    				$job_url = $node->filter('td:nth-child(2) .ProjectList_ProjectLink')->extract(['href']);
                    $job_url = isset($job_url[0])?"https://candidate.hr-manager.net/".$job_url[0] : '';
    	...
    	foreach($data as $value)
    	{
    	    if(isset($value['Job Title']) && $value['Job Title'] !='')
    	    {
                ...
                $new_job->company = 'Roskilde kommune';
