...
class HolbaekKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HolbaekKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Holbæk Kommune';
        $base_url = 'https://holbaek.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://holbaek.dk/om-kommunen/job-i-kommunen/ledige-stillinger/');
        $total_pagination = $crawler->filter('.text-page-text .list-item')->count();
        ...
        $data_25 = $crawler->filter('.text-page-text .list-item')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                $job_url = $node->extract(['href']);
                $job_url = isset($job_url[0]) ? $job_url[0] : '';
                $job_url = 'https://holbaek.dk/om-kommunen/job-i-kommunen/ledige-stillinger/' . $job_url;

                $job_title = null;
                if ($node->filter('.list-item-text h4')->count()) {
                    $job_title = $node->filter('.list-item-text h4')->text();
                }
                ...
                if ($job_title) {
                    ...
                    $job_description_html = $crawler->filter('.text-page-text')->html();
                    $job_description_html = preg_replace('#<h1>.*?</h1>#is', '', $job_description_html);
                    $job_description_html = preg_replace('#<p>.*?</p>#is', '', $job_description_html);

                    $job_description = strip_tags($job_description_html);

                    $temp_infos = $crawler->filter('.hotspot-content .hotspot-text .emply-fact')->each(function ($node) {
                        if ($node->filter('.emply-fact-title')->count()) {
                            if ($node->filter('.emply-fact-title')->text() == 'Telefonnummer') {
                                return ['contact_phone' => $node->filter('.emply-fact-text')->text()];
                            }
                            if ($node->filter('.emply-fact-title')->text() == 'Kontaktperson') {
                                return ['contact_person' => $node->filter('.emply-fact-text')->text()];
                            }
                            if ($node->filter('.emply-fact-title')->text() == 'Ansøgningsfrist') {
                                return ['job_deadline_date' => $node->filter('.emply-fact-text')->text()];
                            }
                        }
                    });
                }
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Holbæk Kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Holbæk Kommune';
