...
class RandersKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:RandersKommuneJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Randers kommune';
	   	$base_url = 'https://www.randers.dk';
    	...
    	$crawler = $goutte->request('GET', 'https://www.randers.dk/job/ledige-job-i-randers-kommune/');
		$logo=$crawler->filter('.new-top__static-logo')->extract(["src"]);
        $logo = isset($logo[0]) ? $base_url . $logo[0] : '';

		$total_pagination = $crawler->filter('.Subpage__joblist ul li')->count();
	    ...
	    $data_25 = $crawler->filter('.Subpage__joblist ul li')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr){
	        ...
	        if($current_pagination - 1 < $index && $current_pagination + 4 >=$index)
            {
    	    	...
    	    	$ddate=date("Y-m-d",strtotime(trim($crawler->filter('.ra-job div:nth-child(3) p')->text())));
    	    	...
    	    	$job_description = trim($crawler->filter('.rte-content')->text());
    	    	...
    	    	$job_description_html_string = $crawler->filter('.rte-content')->html();
    	    ...
    	    if(isset($value['Job Title']) && $value['Job Title'] !='')
    	    {
    			...
    			if($work_city != '')
    			{
                    ...
                    else
                    {
                        ...
                        $place_model->company = 'Randers kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Randers kommune';
