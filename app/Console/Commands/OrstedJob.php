<?php

namespace App\Console\Commands;

use Illuminate\Console\Command;
use Goutte;
use App\Job;
use App\Place;
use App\City;
use File;
use DB;
use App\JobCategory;
use App\Category;
use Goutte\Client;
use GuzzleHttp\Client as GuzzleClient;
use App\JobLocation;
use Illuminate\Support\Facades\Hash;

class OrstedJob extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'cronjob:OrstedJob';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Command description';

    /**
     * Create a new command instance.
     *
     * @return void
     */
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Execute the console command.
     *
     * @return mixed
     */
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Orsted';
        $base_url = 'https://orsted.com';
        $find_arr = ['marts', 'maj', 'juni', 'juli', 'oktober', 'februar', 'januar'];
        $replace_arr = ['march', 'may', 'june', 'july', 'october', 'february', 'january'];
        $data = [];
        $page = 1;

        $logo = '';

        $search_link = 'https://orsted.com/api/jobs/jobList';

        $headers = [
            'User-Agent' => 'Command Prompt',
            'Content-Type' => 'application/json; charset=utf-8',
            'Accept' => 'application/json',
        ];
        $client = new GuzzleClient([
            // 'verify'=>false,
            'headers' => $headers
        ]);

        // echo json_encode([
        //     "draw" => 2,
        //     "columns" => [[
        //         "data" => "availableJobs",
        //         "name" => "",
        //         "searchable" => true,
        //         "orderable" => true,
        //         "search" => [
        //             "value" => "",
        //             "regex" => false
        //         ]
        //     ]],
        //     "order" => [[
        //         "column" => "availableJobs",
        //         "dir" => "asc"
        //     ]],
        //     "start" => 0,
        //     "length" => 150,
        //     "offset" => 0,
        //     "item" => "41d69dfd-861a-4b9b-aa6e-8db172f7ed3e",
        //     "countryFilterID" => "{A86A8D05-3DCC-46B3-99DE-026C83655B58}"
        // ]);
        // exit;

        $r = $client->post($search_link, [
            'body' => '{"draw":2,"columns":[{"data":"availableJobs","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}},{"data":"positioncategory","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}},{"data":"employmenttype","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}},{"data":"country","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}},{"data":"workplace","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}},{"data":"deadline","name":"","searchable":true,"orderable":true,"search":{"value":"","regex":false}}],"order":[{"column":"availableJobs","dir":"asc"}],"start":0,"length":150,"search":{"value":"","regex":false},"offset":0,"item":"41d69dfd-861a-4b9b-aa6e-8db172f7ed3e","country":"Denmark","category":"","jobtitle":"","workplace":"","employmenttype":"","countryFilterID":"{A86A8D05-3DCC-46B3-99DE-026C83655B58}"}'
        ]);
        $jobs_data = json_decode($r->getBody()->getContents());
        $total_pagination = sizeof($jobs_data->data);


        $updated = DB::table('cron_schedule')->where('company_name', $company_name)->update(['page_size' => intval($total_pagination)]);

        $cron_schedule = DB::table('cron_schedule')->where('company_name', $company_name)->first();
        $current_pagination = $cron_schedule->pagination;
        $totalpagesize = $cron_schedule->page_size;
        $checked_status = $cron_schedule->checked;


        if ($totalpagesize <= $current_pagination) {
            $updated = DB::table('cron_schedule')->where('company_name', $company_name)->update(['checked' => intval($checked_status + 1)]);
            $updated = DB::table('cron_schedule')->where('company_name', $company_name)->update(['pagination' => 1]);
            $current_pagination = 1;
        }

        foreach ($jobs_data->data as $index => $jobs) {
            if ($current_pagination - 1 <= $index && $current_pagination + 2 > $index) {
                $job_url = $base_url . $jobs->Link;

                $this->info('job url : ' . $job_url);

                $goutte = new Client();
                // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                $crawler = $goutte->request('GET', $job_url);

                $job_real_url = '';
                if ($crawler->filter('a.job-details-apply')->count()) {
                    $job_real_url = $crawler->filter('a.job-details-apply')->extract(['href']);
                    $job_real_url = isset($job_real_url[0]) ? $job_real_url[0] : '';
                }

                $job_title = $jobs->JobName;

                $job_deadline_date = $jobs->ApplicationDue;

                $this->info('job deadline date : ' . $job_deadline_date);

                $job_deadline_date = date('Y-m-d', strtotime($job_deadline_date));

                $job_parent_category = '';
                $job_sub_category = '';
                $job_category_url = "https://www.jobsearch.dk/jobdatabase?searchText=" . $job_title;

                $goutte = new Client();
                // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                $category_crawler = $goutte->request('GET', $job_category_url);

                if ($category_crawler->filter('a.read-more')->count()) {
                    $job_category = $category_crawler->filter('a.read-more')->extract(['href']);
                    if(isset($job_category[0])){
                        $categoryArr = explode('/', $job_category[0]);
                        $job_parent_category = $categoryArr[2];
                        $job_sub_category = $categoryArr[3];
                    }
                }


                if ($crawler->filter('.job__content-column')->count()) {
                    $job_description_html = $crawler->filter('.job__content-column')->html();
                    $job_description = strip_tags($job_description_html);
                } else {
                    $this->info('job_real_url : ' . $job_real_url);
                    $goutte = new Client();
                    // $goutte->setClient(new GuzzleClient(array('verify' => false)));
                    $crawler = $goutte->request('GET', $job_real_url);


                    if ($crawler->filter('.jobDisplay')->count()) {
                        if ($crawler->filter('.jobDisplay .job')->count()) {
                            if ($crawler->filter('.jobDisplay .job')->text() == 'Sorry, this position has been filled.') {
                                $this->info('============================================================= job expired');
                                continue;
                            }
                        }
                    }

                    $job_description_html = $crawler->filter('.joblayouttoken:nth-child(2)')->html();
                    $job_description = strip_tags($job_description_html);
                }

                $work_city = $jobs->PositionWorkPlace;
                $PositionLocation = $jobs->PositionLocation;
                $locationArr = explode(",", $PositionLocation);

                $city_index = 0;
                foreach ($locationArr as $k => $location) {
                    if (trim($location) == 'Denmark') {
                        $city_index = $k;
                    }
                }
                $cityArr = explode(",", $work_city);
                if (sizeof($cityArr) > 1) {
                    $work_city = trim($cityArr[$city_index]);
                }

                $this->info('work city : ' . $work_city);
                $this->info('===========================================================');
                $job_type = 1;
                $temp_data = array(
                    "Job Title" => $job_title,
                    "Job Description" => $job_description,
                    "Job Description Html" => $job_description_html,
                    "Job Created Date" => '',
                    "Job Start Date" => '',
                    "Job Deadline Date" => date('Y-m-d', strtotime($job_deadline_date)),
                    "Company Logo" => $logo,
                    "Work address" => '',
                    "Work City" => $work_city,
                    "Work Type" => '',
                    "Job Type" => $job_type,
                    "Job Url" => $job_url,
                    "Job Parent Category" => $job_parent_category,
                    "Job Sub Category" => $job_sub_category
                );
                $data_25[] = $temp_data;
            }
        }
        $data = array_merge($data, $data_25);


        $old = false;
        $place_inserted_id = 0;
        foreach ($data as $key => $value) {
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                $work_city = $value['Work City'];
                $work_type = $value['Job Type'];
                $job_title = $value['Job Title'];
                $job_url = $value['Job Url'];
                $job_description = $value['Job Description'];
                $job_description_html = $value['Job Description Html'];
                $job_created_date = $value['Job Created Date'];
                $job_parent_category = $value['Job Parent Category'];
                $job_sub_category = $value['Job Sub Category'];
                $job_type_id = $value['Job Type'];
                $job_deadline_date = $value['Job Deadline Date'];

                $job_location = 0;

                if ($work_city != '') {
                    if ($work_city == 'Hovedstadsområdet') {
                        $work_city = 'København';
                    } else if ($work_city == 'Lyngby') {
                        $work_city = 'Kongens Lyngby';
                    } else {
                    }
                    $city = City::where('name', '=', $work_city)->first();
                    if ($city) {
                        $job_location = $city->id;
                    } else {
                        $place_model = new Place;
                        $place_model->name = $work_city;
                        $place_model->company = 'Orsted';
                        if ($place_model->save()) {
                            $place_inserted_id = $place_model->id;
                        }
                    }
                }

                $job_logo = 'orsted.png';

                $new_job = new Job;
                $new_job->title = $job_title;
                $new_job->seo = str_slug(str_replace($dst, $src, $job_title));
                $new_job->is_redirect = 0;
                $new_job->url = Hash::make(date('Y-m-d H:i:s'));
                $new_job->real_url = $job_url;
                $new_job->description = $job_description;
                $new_job->description_html = $job_description_html;
                $new_job->job_type_id = $work_type;
                $new_job->is_active = 1;
                $new_job->logo = $job_logo;
                $new_job->company = 'Orsted';
                $new_job->company_website = '';
                $new_job->city_id = $job_location;
                $new_job->job_deadline_date = $job_deadline_date;
                $new_job->created_at = date('Y-m-d');
                $new_job->created_on = date('Y-m-d');

                if ($job_title != '') {
                    $existing_job = Job::where('real_url', $job_url)->first();

                    if (!$existing_job) {
                        if ($new_job->save()) {
                            $inserted_id = $new_job->id;
                            if ($place_inserted_id) {
                                Place::where('id', $place_inserted_id)->update(['job_id' => $inserted_id]);
                            }
                            if ($job_sub_category != '') {
                                $category = Category::where('seo', $job_sub_category)->where('level', 2)->first();
                                if ($category) {
                                    $job_category = new JobCategory;
                                    $job_category->job_id = $inserted_id;
                                    $job_category->category_id = $category->id;
                                    $job_category->save();
                                }
                            }
                            if ($job_location) {
                                if (JobLocation::where('job_id', $inserted_id)->where('location_id', $job_location)->first()) {
                                } else {
                                    $new_job_locations = new JobLocation();
                                    $new_job_locations->job_id = $inserted_id;
                                    $new_job_locations->location_id = $job_location;
                                    $new_job_locations->save();
                                }
                            }
                        }
                    } else {
                        $updated = Job::where('real_url', $job_url)->update(['description' => $job_description]);
                    }
                }
            }
        }

        $updated = DB::table('cron_schedule')->where('company_name', $company_name)->update(['pagination' => intval($current_pagination + 3), 'updated_at' => date('Y-m-d H:i:s')]);
        if ($old) {
            $updated = DB::table('cron_schedule')->where('company_name', $company_name)->update(['pagination' => 1, 'updated_at' => date('Y-m-d H:i:s')]);
        }
    }
}
