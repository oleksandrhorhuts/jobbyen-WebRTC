...
class PostNordJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:PostNordJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $address = 'Indgå kontrakter med eksterne partnere om varetagelse af posthuse og Collect Shop m.v.';

        $company_name = 'postnord';
        $base_url = 'https://www.postnord.com';
        ...
        $crawler = $goutte->request('GET', 'https://www.postnord.com/da/karriere/arbejde-hos-os/jobs-i-produktionen?page='.$current_pagination);
        $logo = '';

        $jobs_count = $crawler->filter('table tbody tr')->count();
        if ($jobs_count == 0) {
        ...
        $data_25 = $crawler->filter('table tbody tr')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr) {
            ...
            $job_url = $node->filter('td.job-title a')->extract(['href']);
            ...
            if($crawler->filter('.table-job-detail tbody tr')->count()){
                $temp_infos = $crawler->filter('.table-job-detail tbody tr')->each(function ($node) {
                    if($node->filter('th')->text() == 'Sidst opdateret:'){
            ...
            $job_created_date = '';
            if(sizeof($data_infos)){
                $job_created_date = $data_infos[0];
            }

            if($crawler->filter('.page-pagecontainer .sectioncontainer div')->count()){
                $job_description = $crawler->filter('.page-pagecontainer .sectioncontainer div')->each(function ($node, $index) {
                    if($index == 1){
                        return $node->html();
                    }
                });
            }
            $job_description = array_values(array_filter($job_description));

            $job_description_html_string = '';
            $job_description_string = '';
            if(sizeof($job_description)){
                $job_description_html_string = $job_description[0];
                $job_description_string = strip_tags($job_description[0]);
            }
