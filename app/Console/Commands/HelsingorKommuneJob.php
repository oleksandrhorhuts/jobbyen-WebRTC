...
class HelsingorKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HelsingorKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'HELSINGØR KOMMUNE';
        $base_url = 'https://www.helsingor.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://www.helsingor.dk/om-kommunen/job-og-uddannelse/ledige-job/');
        $total_pagination = $crawler->filter('.list .list-item')->count();
        ...
        $data_25 = $crawler->filter('.list .list-item')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {

                $job_title = $node->filter('.list-title a')->text();
                $job_url = $node->filter('.list-title a')->extract(['href']);
                $job_url = isset($job_url[0]) ? $job_url[0] : '';
                $job_deadline_date = '';
                if($node->filter('.description')->count()){
                    $job_deadline_date = $node->filter('.description')->text();
                    preg_match('/Ansøgningsfrist (.*), (.*)/', $job_deadline_date, $matches);

                    if(sizeof($matches) > 2){
                        $job_deadline_date = str_replace($find_arr, $replace_arr, $matches[1]);
                    }
                }
                ...
                $job_description_html = $crawler->filter('.emply')->html();
                $job_description_html = preg_replace('#<div class=\"emply-buttons\">.*?</div>#is', '', $job_description_html);
                $job_description_html = preg_replace('#<p class=\"teaser-text\">.*?</p>#is', '', $job_description_html);
                $job_description_html = preg_replace('#<a class=\"btn btn-info btn-lg\".*?</a>#is', '', $job_description_html);
                $job_description_html = preg_replace('#<h1>.*?</h1>#is', '', $job_description_html);
                $job_description_html = htmlspecialchars_decode($job_description_html);
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'HELSINGØR KOMMUNE';
                        if ($place_model->save()) {
                ...
                $job_logo = 'helsingor_kommune.png';
                ...
                $new_job->company = 'HELSINGØR KOMMUNE';
