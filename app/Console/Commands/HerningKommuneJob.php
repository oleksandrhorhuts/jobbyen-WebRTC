...
class HerningKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:HerningKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
		$dst = ['ø', 'å', 'æ'];
		$company_name = 'Herning Kommune';
		$base_url = 'https://www.herning.dk/';
		...
		$crawler = $goutte->request('GET', 'https://www.herning.dk/ledige-job');
        $total_pagination = $crawler->filter('.agenda-text li')->count();
        ...
        $data_25 = $crawler->filter('.agenda-text li')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index) {
                ...
                $job_title = $node->filter('.application-title')->text();
                $job_deadline_date = $node->filter('.application-date')->text();
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Herning Kommune';
                        if ($place_model->save()) {
                ...
                $new_job->company = 'Herning Kommune';
