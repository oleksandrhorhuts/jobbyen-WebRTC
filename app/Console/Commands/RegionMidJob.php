...
class RegionMidJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:RegionMidJob';
    ...
    public function handle()
    {
        ...
        $company_name = 'Region Midtjylland';
        $base_url = 'https://www.rm.dk';
        ...
        $crawler = $goutte->request('GET', 'https://www.rm.dk/job/ledige-job/');
        $logo = '';
        ...
        $data_25 = $crawler->filter('table tbody tr')->each(function ($node, $index) use ($base_url, $current_pagination, $logo, $find_arr, $replace_arr) {
            ...
            if ($current_pagination - 1 <= $index && $current_pagination + 4 > $index) {
                ...
                $work_address = trim($node->filter('td:nth-child(2)')->text());

                $arr = explode(",", $work_address);
                $work_city = trim($arr[sizeof(explode(",", $work_address)) - 1]);

                $address = trim($node->filter('td:nth-child(2)')->text());

                $ddate = date("Y-m-d", strtotime(trim($node->filter('td:nth-child(4)')->text())));
                ...
                if ($crawler->filter('.executer_vacancy_text')->count() > 0) {
                    $job_description = trim($crawler->filter('.executer_vacancy_text')->text());

                    $job_description = preg_replace("/\r|\n/", "", $job_description);
                    $job_description = preg_replace("/\t/", " ", $job_description);
                    $job_description = trim($job_description, " \t\n\r\0\x0B\xC2\xA0");

                    $job_description_html_string = $crawler->filter('.executer_vacancy_text')->html();
        ...
        foreach ($data as $value) {
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($work_city == 'Psykiatri') {
                        $work_city = 'Brønshøj';
                    } else if ($work_city == 'Hospitalsenhed Midt') {
                        $work_city = 'Viborg';
                    } else if ($work_city == 'Regionshospitalet Randers') {
                        $work_city = 'Randers';
                    } else if ($work_city == 'Hospitalsenhed Midt') {
                        $work_city = 'Viborg';
                    } else if ($work_city == 'Hospitalsenheden Vest') {
                        $work_city = 'København';
                    } else if ($work_city == 'Aarhus Universitetshospital') {
                        $work_city = 'Århus';
                    } else if ($work_city == 'Regionshospitalet Horsens') {
                        $work_city = 'Horsens';
                    } else if ($work_city == 'Regionshuset Viborg') {
                        $work_city = 'Viborg';
                    } else if ($work_city == 'RKKP') {
                        $work_city = 'Odense';
                    } else if ($work_city == 'Regionshuset Aarhus') {
                        $work_city = 'Århus';
                    } else if ($work_city == 'Hospitalsapoteket') {
                        $work_city = 'Århus';
                    } else {
                ...
                $job_logo = 'region-midtjylland.png';
                ...
                $new_job->company = 'Region Midtjylland';
