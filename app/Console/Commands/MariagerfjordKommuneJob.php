...
class MariagerfjordKommuneJob extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cronjob:MariagerfjordKommuneJob';
    ...
    public function handle()
    {
        $src = ['oe', 'aa', 'ae'];
        $dst = ['ø', 'å', 'æ'];
        $company_name = 'Mariagerfjord Kommune';
        $base_url = 'https://www.mariagerfjord.dk/';
        ...
        $crawler = $goutte->request('GET', 'https://mariagerfjord.emply.net/overview/Mariagerfjord.aspx?mediaId=537a7324-00fa-42c1-822d-d0256092ddb9');
        $total_pagination = $crawler->filter('.jobs_table .jobs_row')->count();
        ...
        $data_25 = $crawler->filter('.jobs_table .jobs_row')->each(function ($node, $index) use ($base_url, $current_pagination, $find_arr, $replace_arr) {
            if ($current_pagination - 1 <= $index && $current_pagination + 3 > $index && $index > 0) {
                $job_url = '';
                if ($node->filter('.jobs_cell:nth-child(1) a')->count()) {
                    $job_url = $node->filter('.jobs_cell:nth-child(1) a')->extract(['href']);
                    $job_url = isset($job_url[0]) ? $job_url[0] : '';
                }

                if ($job_url) {
                    $job_title = $node->filter('.jobs_cell:nth-child(1) a')->text();
                    $job_deadline_date = $node->filter('.jobs_cell:nth-child(2)')->text();
                    ...
                    $sub_details = $crawler->filter('.executer_vacancy_factbox_content')->html();


                    $contact_person = $this->cutString($sub_details, '<b>Kontaktperson:', '<br><b>');
                    if($contact_person){
                        $contact_person = strip_tags($contact_person);
                    }

                    $contact_phone = $this->cutString($sub_details, '<b>Telefon:', '<br><b>');
                    if($contact_phone){
                        $contact_phone = strip_tags($contact_phone);
                    }

                    $work_type = $this->cutString($sub_details, '<b>Ansættelsesvilkår:', '<br><b>');
                    if($work_type){
                        $work_type = strip_tags($work_type);
                    }
            ...
            if (isset($value['Job Title']) && $value['Job Title'] != '') {
                ...
                if ($work_city != '') {
                    ...
                    if ($city) {
                        ...
                        $place_model->company = 'Mariagerfjord Kommune';
                        if ($place_model->save()) {
                ...
                $job_logo = 'mariagerfjord_kommune.png';
                ...
                $new_job->company = 'Mariagerfjord Kommune';
